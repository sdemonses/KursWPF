...
namespace Kursach.Class
{
    public class DeliveryNote
    {
        ...
        public void SavePDF()
        {
            ...
            string fileName = Directory.GetCurrentDirectory() + "\\" + "Накладная" + sourceList.Date.ToString().Replace(":", "-") + "_" + sourceList.Employees.Surname + sourceList.Employees.Name + ".pdf";//Замена ':' на '-' обусловленна тем, что название файла не может содержать ':'
            ...
            PdfPTable table = new PdfPTable(new float[] { 7, 12, 7, 7, 7, 7 });
            ...
            PdfPCell Type = new PdfPCell(new Phrase("Накладная", new iTextSharp.text.Font(baseFont, 24)));
            ...
            string[] Headers = new string[] { "ID", "Наименование", "Цена закупки", "Цена продажи", "Количество", "Сумма" };
            ...
            foreach (DeliveryInfo product in sourceList.DeliveryInfos)
            {
                string name;
                ...
                table.AddCell(new Phrase(string.Format("{0:c}", product.Price), new iTextSharp.text.Font(font.BaseFont, 10)));
                ...
                table.AddCell(new Phrase(string.Format("{0:n}", product.Summa), new iTextSharp.text.Font(font.BaseFont, 10)));
            }
            table.AddCell(dottedSpace);
